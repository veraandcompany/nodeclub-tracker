...
- Usage rollups are **inference-specific**: only assistant turns with `provider`/`providerID` in `PiSessionReader.providers` count (default `["nodeclub"]` = baseUrl `https://api.nodeclub.ai/v1`; override `PI_PROVIDERS`, comma-separated; applies to pi *and* OpenCode). Sessions with no matching turns are skipped entirely (not listed as zero-usage projects). Live-agent detection is **not** provider-filtered (it's pi liveness).
- OpenCode usage comes from `~/.local/share/opencode/opencode.db` (**read-only** SQLite, system lib; override `OPENCODE_DATA_DIR`). Schema drift degrades to an empty snapshot, never a crash. UsageSnapshot is source-agnostic; `UsageSnapshot.merged` combines pi + OpenCode in `PiDashboardModel`.
