...
- `Sources/TaskbarCore`: pure Swift logic — models, stores, formatting. No UI imports; must stay testable without a running app.
- `Sources/Taskbar`: the menu bar app — SwiftUI scenes and views only. No business logic here; views call into `TaskbarCore`.
...
- pi usage parsing lives in `TaskbarCore` as pure functions over JSONL lines; wire-format structs are private and map only the fields we need (assistant `usage` + `cwd` + entry `timestamp`).
- `Tests/TaskbarCoreTests`: XCTest coverage for core behavior; mirror new core logic with focused tests. (XCTest for now — the active toolchain is Command Line Tools, which doesn't ship Swift Testing. Revisit Swift Testing once Xcode's toolchain is the active developer dir.)
- `Scripts`: build/package helpers. `compile_and_run.sh` is the dev loop (kill, build, package, relaunch, verify). `package_app.sh` assembles `Taskbar.app` from the SwiftPM product.
